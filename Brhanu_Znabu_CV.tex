...
My research develops deep learning foundation models, large language models, and generative models for large-scale biological sequence analysis across genomes and proteins, with a focus on domain-adaptive pre-training, generative design, autonomous LLM agents for scientific analysis, and rigorous leakage-aware evaluation. I build systems end to end, from data curation through HPC training to open-source release, and apply them to predicting evolutionary trajectories and protein function.
...
\begin{itemize}[leftmargin=1.2em, topsep=2pt, itemsep=2pt, parsep=0pt]
  \item \textbf{RNA-seq Agent} \href{https://github.com/brhanufen/rnaseq-agent}{(github.com/brhanufen/rnaseq-agent)}: Autonomous LLM agent that plans, writes, and runs Python for bulk RNA-seq analysis, self-corrects on errors, and returns grounded answers, shipped with a reliability benchmark and technical report: on a 10-task benchmark over GSE52778 (3 repetitions each) it solved every task with each reported number traceable to tool output, recovered from all induced errors, and used a median of 3 steps per task. The benchmark also documents that "how many genes are differentially expressed" has no single ground truth, ranging from 552 to $\sim$10,000 across pipelines.
...
\begin{enumerate}[leftmargin=1.5em, topsep=2pt, itemsep=2pt, parsep=0pt]
  \item Ashiquzzaman A*, Lee E*, \textbf{Znabu BF*}, Sakib AN, Chung G, Kim SS, Kim YR, Kwon H-S, Chung E. MoSeq based 3D behavioral profiling uncovers neuropathic behavior changes in diabetic mouse model. \textit{Scientific Reports} 15, 15114 (2025). \href{https://doi.org/10.1038/s41598-025-98184-9}{DOI: 10.1038/s41598-025-98184-9}
...
\begin{itemize}[leftmargin=1.2em, topsep=2pt, itemsep=2pt, parsep=0pt]
  \item \textbf{ISMB 2026} (MLCSB COSI), poster accepted: Investigating Host-Range Determinants of Flaviviruses Using Foundation Model Embeddings: A Leakage-Aware Evaluation Framework. Washington, DC (2026). Plus symposium posters at UNL (2026) and the Nebraska Center for Virology (2025).
\end{itemize}

% ===== TECHNICAL SKILLS =====
\section{Technical Skills}

\textbf{Programming:} Python, R, C++, MATLAB \quad $|$ \quad \textbf{ML/DL:} PyTorch, scikit-learn, XGBoost, Hugging Face Transformers, DNABERT-2, ESM-2, AntiBERTy \quad $|$ \quad \textbf{Infrastructure:} GPU (H100/H200), HPC (Slurm), Git/GitHub \\[3pt]
...
\begin{itemize}[leftmargin=1.2em, topsep=2pt, itemsep=2pt, parsep=0pt]
  \item Biswas Family Foundation Fast Grant (\$25,000, 2026) for \textit{Forecasting Viral Emergence from Sequence: Generative Foundation Models for Arbovirus Surveillance}
